...
describe("LuckyToken config v2 diagnostics cutover", () => {
  ...
  it("accepts only the v2 diagnostics root and resolves its directory from the application-state root", async () => {
    const root = await mkdtemp(join(tmpdir(), "luckytoken-config-v2-"));
    directories.push(root);
    const applicationStateRoot = join(root, ".luckytoken");
    const configPath = join(applicationStateRoot, "config.json");
    await mkdir(dirname(configPath), { recursive: true });
    await writeFile(configPath, JSON.stringify(v2Config()), "utf8");

    const loaded = (await loadLuckyTokenCliConfig(configPath)) as unknown as
      Record<string, unknown>;
    ...
    await writeFile(
      configPath,
      JSON.stringify(v2Config({ schemaVersion: "luckytoken-config-v1" })),
      "utf8",
    );
    await expect(loadLuckyTokenCliConfig(configPath)).rejects.toThrow(
    ...
    for (const key of LEGACY_DIAGNOSTICS_KEYS) {
      ...
      await expect(loadLuckyTokenCliConfig(configPath)).rejects.toThrow(
        `LuckyToken config root has unknown field: ${key}`,
      );
    }
  });
  ...
  it("writes first-run configuration with only the v2 diagnostics defaults", async () => {
    const applicationStateRoot = await mkdtemp(
      join(tmpdir(), "luckytoken-first-run-v2-"),
    );
