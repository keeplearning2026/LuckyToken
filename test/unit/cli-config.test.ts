...
describe("Token CLI configuration", () => {
  ...
  it("configures Client Protocols without any client-auth file", async () => {
    const root = await mkdtemp(join(tmpdir(), "Token-cli-"));
  ...
  it("loads Provider Native Responses retry policy independently from conversion", async () => {
    const directory = await mkdtemp(join(tmpdir(), "Token-cli-native-"));
  ...
  it("rejects server.host because the Data Plane bind address is fixed to loopback", async () => {
    const directory = await mkdtemp(join(tmpdir(), "Token-cli-host-"));
    directories.push(directory);
    const path = join(directory, "config.json");
    await writeFile(
      path,
    ...
    await expect(loadTokenCliConfig(path)).rejects.toThrow("server has unknown field: host");
  });
  ...
  it("defaults the canonical models.json to the config data directory and never to Pi Agent's or the Pi credential directory", async () => {
    const directory = await mkdtemp(join(tmpdir(), "Token-cli-"));
    directories.push(directory);
    const path = join(directory, "config.json");
    await writeFile(
      path,
    ...
    // layout's `~/.Token/models.json`), never inside the Pi credential
    ...
    const explicit = await loadTokenCliConfig(explicitPath);
  ...
  it("preserves raw configuration under validated Provider Package names", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        providerPackages: {
          "@token/provider-commandcode-private": packageConfiguration,
        },
        pi: { directory: "pi" },
      }),
      "utf8",
    ...
    expect(
      config.providerPackages["@token/provider-commandcode-private"],
  ...
  it("preserves a protocol id named __proto__ as data rather than map structure", async () => {
    ...
    await writeFile(
      path,
      '{"schemaVersion":"token-config-v2","clientProtocols":{"__proto__":{}},"pi":{"directory":"pi"}}',
      "utf8",
    );
  ...
  it("refuses an unversioned legacy config instead of migrating or overwriting it", async () => {
    const directory = await mkdtemp(join(tmpdir(), "Token-cli-legacy-"));
    directories.push(directory);
    const path = join(directory, "config.json");
    ...
    await expect(loadTokenCliConfig(path)).rejects.toThrow(
  ...
  it.each([
    [{ schemaVersion: "token-config-v2", clientProtocols: { fixture: {} }, pi: { directory: "pi" }, extra: true }],
    [{ schemaVersion: "token-config-v2", server: { port: 65_536 }, clientProtocols: { fixture: {} }, pi: { directory: "pi" } }],
    [{ schemaVersion: "token-config-v2", clientProtocols: {}, pi: { directory: "pi" } }],
    [{ schemaVersion: "token-config-v2", clientProtocols: { fixture: { authFile: "obsolete.json" } }, pi: { directory: "pi" } }],
    [{ schemaVersion: "token-config-v2", clientProtocols: { fixture: { extra: true } }, pi: { directory: "pi" } }],
    [{ schemaVersion: "token-config-v2", clientProtocols: { fixture: {} }, client: { apiKey: "legacy", projectDir: "legacy-project" }, pi: { directory: "pi" } }],
    [{ schemaVersion: "token-config-v2", clientProtocols: { fixture: {} }, pi: { directory: "" } }],
    [{ schemaVersion: "token-config-v2", clientProtocols: { fixture: {} }, providerAdapters: { "commandcode-private": {} }, pi: { directory: "pi" } }],
    [{ schemaVersion: "token-config-v2", clientProtocols: { fixture: {} }, providerPackages: { "../private-provider": {} }, pi: { directory: "pi" } }],
  ...
  it.each([
    ...
    await expect(loadTokenCliConfig(path)).rejects.toThrow(
      `Token config root has unknown field: ${field}`,
    );
  });
