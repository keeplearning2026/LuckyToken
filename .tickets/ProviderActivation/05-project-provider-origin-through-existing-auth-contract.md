...
**What to build:** The product can distinguish Pi built-in, LuckyToken-bundled, and user Providers through the existing Auth discovery response without creating a second Provider DTO tree or asking the Renderer to infer origin.

**Blocked by:** 02 — Move Provider creation behind one Backend-owned Provider Runtime.

**Status:** ready-for-agent

- [ ] Add RED contract tests for the Provider source values `pi_builtin`, `luckytoken_bundled`, and `user` through the strict Control Plane wire.
- [ ] Auth query projects Provider source alongside existing Pi-derived name/auth metadata and sanitized credential status.
- [ ] The `pi_builtin` Provider IDs returned by Auth query exactly match the pinned Pi `builtinProviders()` identities rather than a hand-maintained LuckyToken list.
- [ ] CommandCode Private is classified as `luckytoken_bundled` without hardcoding its authentication behavior in the UI or Control Plane adapter.
