...
**What to build:** A fresh Token Backend automatically includes CommandCode Private as a product-bundled Provider. Users do not configure or install its npm package, while external user Provider Packages remain separately configurable.
...
- [ ] The bundled package uses the same Token Provider Package contract and Pi Provider registration path as other Provider Packages; no protocol or request path special-cases CommandCode.
