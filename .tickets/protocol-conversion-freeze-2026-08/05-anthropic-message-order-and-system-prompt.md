...
- [x] `text A → tool_result X → text B` produces `User(A) → ToolResult(X) → User(B)` without rejection or reordering.
- [x] Empty ordinary segments produce no synthetic messages; safe adjacent-role merging never crosses a ToolResult boundary.
- [x] Top-level `system` string/blocks form the initial Pi systemPrompt with the frozen newline rule.
- [x] Only the first message-level `role=system` is appended to Pi systemPrompt; later message-level system entries become user messages in original order.
- [x] Non-text blocks in a message-level system entry do not gain system privilege and follow their normal mapping.
- [x] A final assistant prefill is accepted as ordinary historical AssistantMessage and emits an Anthropic-local degradation notice.
- [x] Source message/content objects are not mutated and temporary state cannot leak across concurrent requests.
- [x] Tests assert complete Pi message order and systemPrompt text through the converter interface.
- [x] Existing fixture tests are updated to assert semantics, not only acceptance.
- [x] No Provider or Responses vocabulary/import appears in the Anthropic request module.
