...
import type { ReasoningAdapter } from "./contract.js";

type ProjectInput = Parameters<ReasoningAdapter["projectPayload"]>[0];

export class InvalidReasoningProjection extends Error {
  readonly kind = "InvalidReasoningProjection";

  constructor(message: string) {
    super(message);
    this.name = "InvalidReasoningProjection";
  }
}

function record(value: unknown, api: string): Record<string, unknown> {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new InvalidReasoningProjection(`${api} payload must be an object`);
  }
      ...
      kind === "array"
        ...
        : kind === "object"
          ...
          : kind === "true"
            ...
            : typeof value === kind;
    if (!valid) {
      throw new InvalidReasoningProjection(
...
function outcome(
  ...
  value: ProjectionOutcome,
): ReasoningOutcome {
  return Object.freeze({ subject, outcome: Object.freeze(value) });
}

function repairedEffort(input: ProjectInput): ReasoningOutcome {
...
function finish(
  payload: Record<string, unknown>,
  input: ProjectInput,
  controls: readonly ReasoningOutcome[],
): ReasoningProjectionResult {
...
function unsupportedSummary(input: ProjectInput): ReasoningOutcome | undefined {
...
export function projectOpenAIResponsesPayload(
  ...
  const controls: ReasoningOutcome[] = [];
  const effort = input.prepared.request.effort;
