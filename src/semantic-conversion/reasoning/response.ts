...
export type ResponseContinuityValue =
  | {
...
export type ResponseContinuityBlock =
  | {
      readonly target: "thinking" | "text";
      readonly contentIndex: number;
      readonly source: ReasoningSource;
      readonly continuity?: ResponseContinuityValue;
    }
  | {
...
export interface ReasoningResponseExtraction {
  readonly source: ReasoningSource;
  readonly blocks: readonly ResponseContinuityBlock[];
}
...
function sourceOf(message: AssistantMessage): ReasoningSource {
...
export function extractReasoningResponse(
  message: AssistantMessage,
): ReasoningResponseExtraction {
  const source = sourceOf(message);
  const blocks: ResponseContinuityBlock[] = [];
  for (const [contentIndex, block] of message.content.entries()) {
    if (block.type === "thinking") {
      let continuity: ResponseContinuityValue | undefined;
      if (
