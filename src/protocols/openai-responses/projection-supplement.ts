...
function control<T>(
  value: T,
  requirement: ProjectionControl<T>["requirement"],
): ProjectionControl<T> {
...
function parseFormat(value: unknown): StructuredOutputFormat | undefined {
...
function parseAllowedTool(value: unknown, field: string): SemanticAllowedTool {
  if (!isRecord(value)) {
...
export function parseSemanticToolChoice(
  value: unknown,
...
export function parseResponsesProjectionSupplement(
  ...
  if (includeValue !== undefined && includeValue !== null) {
    ...
    include = Object.freeze([...includeValue]) as readonly string[];
  }
  let topLogprobs: number | undefined;
  if (request.top_logprobs !== undefined && request.top_logprobs !== null) {
    if (
      !Number.isSafeInteger(request.top_logprobs) ||
      (request.top_logprobs as number) < 0 ||
      (request.top_logprobs as number) > 20
    ) {
      throw new InvalidResponsesProjectionSupplement(
        "top_logprobs must be an integer within 0 through 20",
      );
    }
    topLogprobs = request.top_logprobs as number;
  }
  const parallelCalls = optionalBoolean(
    request.parallel_tool_calls,
    "parallel_tool_calls",
  );
  const toolChoice = parseSemanticToolChoice(request.tool_choice);
  ...
  const background = optionalBoolean(request.background, "background");
  const store = optionalBoolean(request.store, "store");
  let contextManagement:
    | readonly Readonly<Record<string, unknown>>[]
    | undefined;
  if (request.context_management !== undefined && request.context_management !== null) {
    if (
      !Array.isArray(request.context_management) ||
      request.context_management.some((entry) => !isRecord(entry))
    ) {
      throw new InvalidResponsesProjectionSupplement(
        "context_management must be an array of objects",
      );
    }
    contextManagement = Object.freeze(
      request.context_management.map((entry) => cloneFrozenRecord(entry)),
    );
  }
  let streamOptions: Readonly<Record<string, unknown>> | undefined;
  if (request.stream_options !== undefined && request.stream_options !== null) {
    if (
      !isRecord(request.stream_options) ||
      Object.keys(request.stream_options).some(
        (key) => key !== "include_obfuscation",
      ) ||
      (request.stream_options.include_obfuscation !== undefined &&
        typeof request.stream_options.include_obfuscation !== "boolean")
    ) {
      throw new InvalidResponsesProjectionSupplement(
        "stream_options must contain only a boolean include_obfuscation",
      );
    }
    streamOptions = cloneFrozenRecord(request.stream_options);
  }
  ...
  const output =
    ...
    topLogprobs === undefined
      ? undefined
      : Object.freeze({
          ...(format === undefined ? {} : { format: control(format, "hard") }),
          ...(verbosity === undefined
            ? {}
            : { verbosity: control(verbosity, "preference") }),
          ...(include === undefined
            ? {}
            : { include: control(include, "preference") }),
          ...(topLogprobs === undefined
            ? {}
            : { topLogprobs: control(topLogprobs, "hard") }),
        });
  const tools =
    parallelCalls === undefined && toolChoice === undefined
      ...
      : Object.freeze({
          ...(parallelCalls === undefined
            ? {}
            : {
                parallelCalls: control(
                  parallelCalls,
                  parallelCalls ? "preference" : "hard",
                ),
              }),
          ...(toolChoice === undefined
            ...
            : {
                choice: control(
                  toolChoice,
                  toolChoice.kind === "auto" ? "preference" : "hard",
                ),
              }),
        });
  const sampling =
    maxOutputTokens === undefined && temperature === undefined && topP === undefined
      ...
      : Object.freeze({
          ...(maxOutputTokens === undefined
            ? {}
            : { maxOutputTokens: control(maxOutputTokens, "hard") }),
          ...(temperature === undefined
            ? {}
            : { temperature: control(temperature, "preference") }),
          ...(topP === undefined ? {} : { topP: control(topP, "preference") }),
        });
  const cache =
    promptCacheKey === undefined && retention === undefined
      ...
      : Object.freeze({
          ...(promptCacheKey === undefined
            ? {}
            : { key: control(promptCacheKey, "preference") }),
          ...(retention === undefined
            ? {}
            : { retention: control(retention, "preference") }),
        });
  const identity =
    safetyIdentifier === undefined && deprecatedUser === undefined
      ...
      : Object.freeze({
          ...(safetyIdentifier === undefined
            ? {}
            : { safetyIdentifier: control(safetyIdentifier, "preference") }),
          ...(deprecatedUser === undefined
            ? {}
            : { deprecatedUser: control(deprecatedUser, "preference") }),
        });
  const lifecycle =
    ...
    streamOptions === undefined
      ...
      : Object.freeze({
          ...(serviceTier === undefined
            ? {}
            : { serviceTier: control(serviceTier, "preference") }),
          ...(truncation === undefined
            ? {}
            : { truncation: control(truncation, "hard") }),
          ...(background === undefined
            ? {}
            : { background: control(background, background ? "hard" : "preference") }),
          ...(store === undefined ? {} : { store: control(store, "preference") }),
          ...(contextManagement === undefined
            ? {}
            : { contextManagement: control(contextManagement, "hard") }),
          ...(streamOptions === undefined
            ? {}
            : { streamOptions: control(streamOptions, "preference") }),
        });
