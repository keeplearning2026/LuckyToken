...
本文件描述 Token 的 Core Architecture。
...
>
> - **Token Core**：Token 的路由核心，不是完整的 Agent、UI、CLI、TUI 或会话产品。它负责接收一次模型请求、完成必要的 protocol conversion 与 runtime execution，再返回结果。
...
> - **Runtime Contract**：运行时模块之间直接使用的类型和行为 contract。Token 直接使用 Pi 的 `Model`、`Context`、`ModelsSimpleStreamOptions`、`AssistantMessageEventStream`、`AssistantMessage` 等，而不是再造等价的通用 IR。
...
Token 不重新实现 LLM runtime，也不建立与 Pi 平行的通用 request/message/response semantic model。
...
never becomes a generic Token request object
```
...
Token 当前 Core text-generation baseline 直接使用 Pi public runtime contracts：
...
但这里的 `Options` 不是 Token 自己定义的新类型。它表示 Token 调用：
...
Pi `Models` 随后仍会根据自身 public runtime contract 处理 Provider auth、effective headers、environment、base URL 等 Provider-facing request preparation。这些属于 Pi runtime ownership，不构成 Token 的第二套 request model。
...
`AssistantMessage` 本身不等于 Token success，因为 Pi 的 `error` terminal 同样携带一个 error-shaped `AssistantMessage`。

Token completed success 明确定义为：
...
Token execution failure 的最小 downstream classification 仍然是：
...
Token 连接三个方向不同的 contract boundary：
...
Token Core 包含完成 Router request 所必需的：
...
Provider authentication、credential handling、Provider dispatch 和 Pi runtime-level model management 优先使用 Pi public runtime capabilities，而不是由 Token 建立平行 infrastructure。
...
即使某个 Client Protocol 使用 streaming-shaped response format，也不代表 Token 当前支持 live token forwarding。当前 contract 只允许在 Pi `done` 后，根据完整 `AssistantMessage` 渲染 protocol-compatible response representation。
...
Pi / Pi Agent 中存在某个 subsystem，不构成 Token Core 必须采用它的理由。
...
Token 优先选择能够正确满足已证明 requirement 的最小 architecture。
...
这不意味着所有 failure 必须进入统一 enum 或 `TokenError`。
...
Token 有两个 fundamentally different conversion direction：
...
当 Token 必须合成这类 structural state 时，representation 应满足：
...
当 semantic request 没有变化时，Token 应避免向 model-visible 或 continuity-sensitive input 引入无关 instability。
...
Pi 是 Token 的 model runtime。

在为以下能力增加 Token-owned infrastructure 之前：
...
而不是 Token 必须继承的 runtime architecture。
...
Generic Token concept 应能够承受一个 unrelated Client Protocol / Provider 的新增或删除，而不吸收那个 integration 的 vocabulary 或 lifecycle。
...
Token-owned module 的 architecture contract：
...
而不是套用 Token-owned `Module Contract` 重新定义其 constructor。
...
Token-owned modules 不得通过以下方式绕过 declared dependency closure。
...
如果以下值参与 Token-owned responsibility 的行为决定：
...
但 framework/library 已经明确规定的 implementation default 不会自动变成 Token architecture abstraction。

例如 Pi contract 允许 Provider adapter 在未注入 custom `fetch` 时使用其 own default transport behavior；Token 不需要为此包装每一个 library primitive。

判断标准是：

> **Does this ambient value participate in a Token-owned responsibility or semantic/runtime decision?**
...
> - **HTTP Boundary**：Token 与 HTTP transport 相接的 boundary。它 owns method/path、raw body/headers、route selection、request `AbortSignal`、connection state 和 final response write。
...
Token Core 有两个主要 information lifetime：
...
对于 Token-owned objects，composition root 建立和绑定 direct dependencies。
...
```text
construct Token-owned runtime objects
...
它们不应被 architecture diagram 错写成 Token 永远必须单独 construction 的 sibling runtime modules。
...
→ Token → selected upstream authentication
...
`HTTP Boundary` 是 HTTP transport 与 Token semantic/runtime boundaries 的接口。
...
`protocol-derived invocation controls` 不是新的 Token IR。
...
> **Concept Primer — 本章说明 Token 如何复用 Pi runtime**
>
> - **Pi Runtime**：Token 使用的 model runtime。它已经拥有 model、Provider、auth、credential、stream 等 runtime responsibilities。
...
Token 使用 `@earendil-works/pi-ai` 作为 model runtime。
...
本章只定义 Generic Token ↔ Pi runtime boundary，不定义：
...
Token Core 直接依赖 Pi public runtime contracts。
...
Pi `Models` 是 external/runtime dependency owned by Pi，Token 不重新定义它的 constructor contract。
...
这不是 Token 对 Pi 的新 abstraction；只是准确描述 current Pi public/source contract。
...
如果 Token 提供 custom `CredentialStore` / `ModelsStore`：
...
Token 把一个 Pi Provider identity 看成一个 concrete runtime integration，而不是单纯 upstream brand name。
...
Token-specific integration 也使用同一个 Pi `Provider` contract：

```text
Token-specific integration
        │
        ▼
    Pi Provider
        │
...
Custom Provider 不构成建立第二个 Token-wide Provider framework 的理由。
...
The versioned `TokenProviderPackage` construction seam is not a
...
只有多个真实 integration 证明存在 Pi `Provider` 无法正确表达的共同 capability 时，才考虑 broader Token abstraction。
...
当一个已证明 Token capability 无法通过 Pi public API 获得时，才检查 Pi Agent：
...
不是 Token 的 runtime architecture template。
...
一个 request 可以在 source protocol 中完全 valid，但仍无法正确表示为 Token 当前使用的 Pi contracts。
...
> **For Token failure ownership, the Pi execution phase begins when Token invokes `Models.streamSimple(...)` and adopts the returned `AssistantMessageEventStream` as the authoritative outcome channel.**
...
在 Pi execution 开始后，Token 保留 Pi `AssistantMessageEventStream` public terminal contract 直接暴露的 distinction：
...
ordinary visible control flow 更符合 Token Core 的 architecture。
...
Token 不再保留一个全局或 per-Client-Protocol 的 client-token Auth boundary。当前 request edge 把四类事实严格分开：
...
Pi `ModelsSimpleStreamOptions` 提供的 capability 不等于 Token Core 自动支持全部 capability。
...
也就是 Token → Pi `Models` 的 invocation input。
...
因此不要把 Token composition output 称为：
...
Pi option field 存在，不代表 unrelated Token state 可以塞进去。
...
一个 Client header 只有在 owning HTTP/Protocol boundary 把它解释成受支持 Token concept 后，才可以通过 semantics 真正匹配的 contract 继续传播。

这保护：

```text
Client → Token authorization / protocol state
```

与：

```text
Token → Provider runtime controls
```
...
Pre-execution failure 不被强制转换成统一 `TokenError` enum。
...
从这个调用点开始，Token 把返回的 `AssistantMessageEventStream` 作为 authoritative outcome channel。
...
Pi public terminal union 包含 `deferred`，但 Token Core v1 的 supported invocation subset **不启用 deferred execution**。
...
Token Core v1 supported execution outcomes
├── completed success
...
但是这只是一条 **defensive correctness rule**，不是 Token 检测所有 malformed Provider lifecycle 的 primary liveness mechanism。
...
Token 直接使用 Pi `Provider` 作为 generic Provider runtime contract，不增加第二层：
...
Pi `Provider` is the generic external runtime contract used by Token.

Pi-owned Provider implementations remain Pi-owned. Token does not redefine their internal dependency closure.

When Token implements a concrete Provider itself, that implementation is a Token-owned module and must satisfy the Module / Operation Runtime Closure rules in Chapter 2.

### Token-Owned Concrete Provider Module Shape
...
Token-owned concrete Provider 的 direct dependencies 必须按 concrete integration 声明。
...
Token normal path 不能依赖 Provider compatibility：
...
的 paths，确保 Token accepted `Context` 不依赖 repair behavior 才变成 valid。
...
> - **CommandCode Private Protocol**：Token 当前基于 observed behavior 推断出的 private `/alpha/generate` wire protocol。它是 observed private contract，不是 CommandCode product 的 universal protocol。
...
Token 当前维护一个 concrete Pi Provider，由私有 workspace/npm package 交付：
...
这两个 wire branch 保持 conceptually distinct，但 Token 不再需要一条额外的 non-Pi request carrier。Current integration 的 Provider input 已经完整表达为：
...
Their presence does not justify Token-wide memory, taste, or skills semantic models.
...
Its presence does not justify a Token-wide permission subsystem.
...
Provider request object 是 temporary transport state，不是 new Token canonical request。
...
Current source basis 包括 CommandCode `/alpha/generate` source-backed protocol document、current Pi `ModelsSimpleStreamOptions` contract，以及 Token 已冻结的 request-identity / credential isolation boundaries。
...
不是 Token composition root 永远必须显式 construction 的 sibling modules。
...
Pi Models / Provider auth
→ Token-to-upstream credential resolution and request preparation
```
...
Token uses Pi's public runtime contracts rather than redefining them:
...
Token Core's primary inference dependency is `Models.streamSimple(...)`, but that does not collapse the entire Pi `Models` API into one operation。
...
Pi execution begins when Token invokes:
...
Token 保持三种 specification responsibility 分离：
...
The following block records the former per-handler Auth design. v6.0 removes that Token client-token/Auth boundary; keep this block only as architectural history.

```text
Token Core Architecture v5.6
        │
...
Token Core Architecture v5.6 冻结 per-handler Auth isolation，同时保留
...
Token Core Architecture v5.7 冻结 Upstream HTTP Failure Preservation，同时
