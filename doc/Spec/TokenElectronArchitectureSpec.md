# Token Electron Product Architecture Specification v1.0

**Status:** ACCEPTED TARGET ARCHITECTURE — implementation pending  
**Date:** 2026-08-18  
**Scope:** Token product/runtime boundaries, Electron desktop, local management transport, testing seams, and migration from the current Tauri desktop  
...
- [Token implementation architecture map](../TokenArchitecture.md)
...
It does **not** redefine Token protocol semantics. The existing Core rule remains authoritative:
...
Token production code is TypeScript/JavaScript plus web UI assets.
...
`TokenRuntime` already exposes the correct deep request seam:
...
All management of a running Token instance flows through the versioned Application Control Plane.
...
```text
Process A — Token Backend
  Node.js / TypeScript
  always running while Token is active

Process B — Electron Main
  tray + desktop lifecycle
  normally always running while Token is active
...
The production authority is a dedicated Token-owned local SQLite lock carrier:
...
The authority has no stale timeout, heartbeat, PID probing, steal operation, owner metadata, or file deletion. Its lock carrier must remain on Token-owned local filesystem storage and must not be reused as a business database, backup source, diagnostics source, or generic file-scanning target. Production Backend discovery belongs to that same current-user application domain (`~/.Token/control-plane.json`) and is not a configurable `serve` argument; only management clients may accept an explicit descriptor path for navigation.
...
export function startTokenApplication(
  options: StartTokenApplicationOptions,
): Promise<RunningTokenApplication>;
```
...
1. discovery descriptor is stored in the current user's private Token data directory;
...
window.Token.invoke(channel, payload)
window.Token.send(channel, payload)
```
...
Its arguments/results reuse `@token/application-control-plane` semantic types.
...
Preload exposes only `TokenDesktopApi` through `contextBridge`.
...
Token Desktop is tray-first by default. Electron Main, the Tray, and the
...
Installed/release Token and repository-generated `.electron-out` builds are
...
Electron `ready` event. This isolation is shell-only: authoritative Token
configuration, credentials, ledger, Control Plane descriptor, and other Backend
state remain under the product-owned `~/.Token` paths.
...
the local Electron exit path directly. Conversely, the tray `Quit Token`
...
Token desktop build is explicitly started.
...
Auto-start. On startup they remove stale per-user Token login items whose
...
Installed/release builds use a stable Token login-item identity. On startup,
Main examines the existing per-user Token launch items. If an older executable
...
multiple stale Token startup entries from accumulating.
...
| `src/cli.ts::runServe()` | extract into `startTokenApplication()`; CLI keeps argument/result presentation only |
...
Renderer tests receive a fake `TokenDesktopApi`.
...
- `TokenRuntime` request interface;
...
At that point Token has one business language, one model-serving semantic core, one running-application management contract, and one replaceable desktop product adapter.

---

# 23. Primary external references

These references justify only platform/runtime mechanics; Token module boundaries are project decisions defined by this specification.
