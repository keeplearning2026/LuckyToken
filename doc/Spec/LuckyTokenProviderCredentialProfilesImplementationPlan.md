# LuckyToken Provider Credential Profiles Implementation Plan v1.5

**Status:** FROZEN — READY FOR TDD IMPLEMENTATION

**Date:** 2026-08-22

**Source requirements:** [LuckyToken Provider Credential Profiles PRD v1.5](./LuckyTokenProviderCredentialProfilesPRD.md)
...
| Catalog runtime handle | Run only explicit/existing Provider model rechecks under an exact managed-or-ambient binding | Narrow refresh operation plus served model snapshot | Never runs because `activeCredentialId` changed; keeps credential scope out of Catalog Controller |
...
The production Store uses the current proven file mechanics: directory mode `0700`, file mode `0600`, a per-file cross-process lock, a restrictive temporary sibling, and atomic rename. It also uses a cross-process per-credential refresh lock where the platform supports the production file-lock contract. A Provider file lock protects only bounded local read/commit work and is never held through OAuth network I/O. It never creates or locks an unrelated Provider record while modifying one Provider.

`listProviderIds()` owns persisted record discovery. The Management Module queries the union of runtime Provider IDs and persisted Provider IDs. A valid stored record for a Provider whose implementation is no longer installed remains visible with `Provider implementation unavailable`; local query and `Remove from LuckyToken` remain available, while login, reconnect, recheck, and request execution fail with a typed unavailable outcome. This prevents a stored secret from becoming undiscoverable or undeletable.
...
For `AmbientBinding`, `read(providerId)` intentionally returns `undefined` and `list()` exposes no managed entry, allowing Pi's existing environment/config/Provider ambient resolution to run. Ambient `modify()` and `delete()` are forbidden because LuckyToken does not own that source. `delete(providerId)` is otherwise permitted only under an exact explicit removal binding and removes only that managed Profile; ordinary local removal calls the Management Interface directly.

This preserves Pi's contract while changing LuckyToken's mapping:

```text
Pi view:        providerId -> one Credential
LuckyToken:     providerId + bound credentialId -> exact Profile payload
```
...
`Models.getAuth()` is allowed only when an exact Profile is actually consumed by a Provider-backed request or explicit recheck. It may perform Pi's silent network token refresh, but it has no `AuthInteraction` and cannot start login. A refresh failure ends that operation and projects only bounded health evidence. It never calls `Models.login()`.
...
## 4.5 Active switch and explicit Catalog refresh
...
The existing Catalog lifecycle still needs a narrow binding-aware runtime handle because model refresh can consume auth. It captures the then-current managed or ambient binding only when an existing lifecycle action or explicit user recheck requests Provider model facts. Changing `activeCredentialId` does not schedule it. The handle may call Provider model discovery and Pi's silent token refresh for a managed OAuth binding, but never `Models.login()` or an interaction callback.

Explicit Catalog capture remains atomic: new requests see refreshed model facts, while in-flight requests keep their already captured `Model` and credential binding. Recheck failure leaves the last usable Catalog snapshot and committed active pointer unchanged and returns a sanitized warning. Switch success never waits on, depends on, or rolls back because of Catalog state.
...
- destructive confirmations distinguish local `Remove from LuckyToken` from OAuth `Disconnect from LuckyToken`, use the Provider-declared auth label, and never claim remote revocation;
...
Every state-reaching test creates a new temporary `CODEX_HOME`, copies only `config.toml` and `luckytoken-model-catalog.json` when required, creates all other fixtures explicitly, passes that path to every spawned process, and removes it in `finally`. It never copies or writes the user's `auth.json`, Profile records, caches, sessions, logs, or credentials.
...
3. Remove one Profile after a `Remove from LuckyToken` confirmation and leave its sibling unchanged.
4. Run OAuth `Disconnect from LuckyToken`/Reconnect through the explicit interaction flow, use the Provider auth-method label, and show ambient eligibility after last removal without claiming remote revocation.
...
An existing `auth.json` remains untouched obsolete user-owned state. Release notes state that LuckyToken ignores it and describe optional manual removal.
...
22. every new behavior is backed by a seam-level test observed failing before implementation, Native wire expectations come from independent literal fixtures, and all guarded test/release gates pass.
