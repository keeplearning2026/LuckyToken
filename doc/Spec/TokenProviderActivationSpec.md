# Token Provider Activation Specification v1.0
...
This specification fixes the current product-level Provider activation gap without changing Token protocol semantics.
...
Current Token also has independent Direct Mode and Provider Native preservation lanes; this Provider Activation specification does not redefine those lane contracts.

The problem addressed here is not protocol conversion. The current Electron product can start, render pages, and expose typed management APIs, but Provider discovery/authentication is coupled to the HTTP Data Plane lifetime. In a fresh or failed/stopped Gateway state the user therefore cannot reliably discover or log in to Providers. In addition, the shipped `@token/provider-commandcode-private` package is installed in the release Backend but is not part of the default product Provider composition.
...
A fresh installed Token must support this flow without editing configuration files:
...
The release assembly installs `@token/provider-commandcode-private`, but the first-run configuration does not configure that package and the current Provider package loader only loads entries present in `config.providerPackages`.
...
`src/application.ts` currently creates `createConfiguredTokenComposition()` inside `DataPlaneRuntimeSupervisor.startListener()`.
...
Token does not maintain a second hardcoded list of Pi Providers.
...
When Pi adds or removes a built-in Provider, Token Provider discovery follows automatically after the Pi version is intentionally upgraded.

## 3.3 CommandCode Private is a Token bundled Provider
...
Token only needs one small additional product fact: where a Provider identity came from.
...
`@token/provider-commandcode-private` and
`@token/provider-commandcode-goat` are bundled product Providers.
...
Token must not maintain a hand-authored list of default aliases for selected models.
...
Do not pass the whole `TokenCliConfig`; server ports, client protocols, diagnostics paths, and unrelated configuration do not belong to the Provider module.
...
3. Token bundled Provider Packages
...
```text
bundled Provider ID   → token_bundled
...
The existing `createConfiguredTokenComposition()` currently mixes Provider-domain creation with Data Plane creation.
...
For alias-only Token client paths, `/v1/models`, Codex catalog generation, and model selection use the one effective alias from Alias Authority.
...
Do not leave a legacy `createConfiguredTokenComposition()` path that can still create a second Pi Models collection in production.
...
CommandCode Private/Goat → token_bundled
...
Using fake `TokenDesktopApi` only:
...
Replace Provider creation inside `createConfiguredTokenComposition()` with injected `Models` and minimal credential scrub capability.
...
- [ ] CommandCode Private is labeled as token-bundled without UI special-casing its authentication flow.
...
A selected-model table means newly available Pi/bundled/user models can exist without any usable alias and forces Token to maintain model identity data that the Catalog already owns.
...
Token's alias-only serving contract already has one-effective-alias-per-target semantics. Keeping multiple aliases per target adds collision, discovery, client-catalog, and edit/delete semantics that are not required by the product request.
