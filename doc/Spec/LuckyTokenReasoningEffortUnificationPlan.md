# LuckyToken Reasoning Effort Unification Implementation Plan
...
The completed change has two shared authorities, and no new LuckyToken-wide effort module:
...
- no LuckyToken implementation reimplements upward-then-downward nearest-level selection;
...
LuckyToken routed rows expose exactly five Codex slots. The JSON `effort` field uses the lowercase Codex key; Light, Medium, High, Extra high, and Ultra are the corresponding user-facing slots.
...
5. Routed rows never inject `minimal`, `off`, or `ultra` as Codex effort keys. Responses input may still normalize the separate LuckyToken `ultra` alias to Pi `max`; that alias is not a sixth routed Codex slot.
...
3. [Anthropic Semantic Conversion Architecture Specification](./LuckyTokenAnthropicSemanticConversionArchitectureSpec.md)
4. [Semantic Conversion Architecture Specification](./LuckyTokenSemanticConversionArchitectureSpec.md)
5. [Codex Model Catalog Specification](./LuckyTokenCodexModelCatalogSpec.md)
...
Responses and Anthropic may both call the same pinned Pi functions, but they do not import one another's intent, plan, Adapter input, outcome, error, or execution Module. No shared LuckyToken effort kernel or shared outcome type is introduced.
...
LuckyToken tests freeze this dependency behavior, including absent and partial `thinkingLevelMap` cases.
...
Examples that freeze Pi selection, not a LuckyToken copy of it. Each row uses a full explicit map: the named non-`off` keys map to identity strings, every unnamed non-`off` key is `null`, and `off` is `null`.
...
| Completion   | `rg` finds no LuckyToken upward/downward level traversal; final-wire tests prove exact, repaired, and degraded behavior; every effort-capability mismatch dispatches a request satisfying I1-I3                                                                 |
...
| Item       | Content                                                                                                                                                                                                                                                                                           |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Contract   | update `LuckyTokenCodexModelCatalogSpec.md` so routed reasoning derives `Provider facts → Pi supported key → five-slot Codex key`, using section 1.2                                                                                                                                              |
