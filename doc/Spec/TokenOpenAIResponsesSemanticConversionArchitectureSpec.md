...
`metadata`, `previous_response_id`, and `store` are consumed for Client response/state
semantics only. They are not Provider controls. Any other top-level field remains
unparsed and produces a bounded request-local warning.
...
Response state never enters Pi Context/options and never claims a Provider-native field
was written. It exists solely to construct a protocol-valid Responses result.
...
| `metadata` | `client-render-or-continuity-state` | validated Client echo only |
...
The renderer accepts a Pi `AssistantMessage` and Responses-owned render/continuity
state. It restores text, thinking, function/custom calls, usage, metadata echo, and
opaque continuity only through defined Responses fields.

Tool choice and temperature fields are normalized Client response echoes. They are not
derived from Provider Wire inspection. Unsupported controls remain the protocol default
or `null`.
