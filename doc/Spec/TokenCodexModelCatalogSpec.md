...
Token must generate a Codex catalog that is both:

1. structurally accepted by the installed Codex runtime; and
2. semantically honest about each routed model and Token's Responses conversion path.
...
| Routed identity and callability | The selected Token Agent injection scope and resolved Pi `Models` collection |
...
- Pairs `tool_mode: "code_mode_only"` with its own deferred-tool execution path. Token does not currently implement that path, so copying this pair would be incorrect: [routed tool discovery](../../reference/opencodex/structure/03_catalog-and-subagents.md#routed-tool-discovery-and-hosted-search).
...
| Field | Required shape | Token value |
| --- | --- | --- |
...
| `support_verbosity` | boolean | `false` until Token preserves `text.verbosity` end to end |
...
For routed Token entries, `base_instructions` is the selected source. `model_messages` is not copied from a native model.

`description` is optional in the 0.149.0 parser; omission normalizes to `null`. Token should still emit a stable alias-oriented description for picker usability.
...
**Confirmed before the generator correction:** the existing file at `<CODEX_HOME>/token-model-catalog.json` contained routed non-reasoning rows without `supported_reasoning_levels`. A real isolated CLI invocation failed with:
...
| --- | --- | --- |
| `description` | `Token model: <alias>` | May use another stable Token-owned alias description; do not expose credentials or private target state |
| `base_instructions` | Token neutral fallback: `You are Codex, a coding agent powered by the selected model.` | Reuse the installed Codex behavioral harness only after replacing native GPT/OpenAI identity claims with the neutral Token identity |
...
Runtime contract: on `codex-cli 0.149.0`, the built-in `openai` provider may first attempt a WebSocket Responses connection even when the routed catalog says `prefer_websockets: false`. Token deliberately leaves WebSocket `/v1/responses` unsupported: it receives the existing `426 Upgrade Required` wire with code `websocket_transport_not_supported`, and Codex retries the turn over HTTP. The same listener always accepts the explicitly supported Codex Realtime `/v1/live` and `/v1/realtime` upgrade shapes. Other WebSocket paths retain the 426 behavior, non-WebSocket upgrades retain the prior close behavior, and ordinary HTTP requests remain unchanged.
...
- `use_responses_lite`;
- `tool_mode: "code_mode_only"` while Token rejects deferred `tool_search`/`defer_loading` lifecycles;
...
Generation is deterministic and the Token-owned file is atomically rewritten on every injection/sync.
...
1. Resolve the current Token endpoint, selected `Favorite` or `Full` scope, Public Model generation, native snapshot, and routed entries.
...
```powershell
$probeHome = Join-Path $env:TEMP "Token-codex-catalog-probe"
...
This is the release gate and must use a newly launched real Codex CLI process against a running Token Data Plane. It must read the injected configuration; do not override `model_provider`, `openai_base_url`, or `model_catalog_json` on the command line.

Minimum scenarios:

1. Non-reasoning routed alias returns a non-empty final agent message with the expected marker.
2. A model with an exact reasoning ladder completes at one advertised effort, and Token's captured request proves the expected effective reasoning value.
...
- Token capture shows the same public alias resolving to the expected canonical target;
- the upstream response completed rather than a fixture or cached response;
- no catalog/config parser warning occurred.

The existing real-client harness is reusable evidence for wire behavior: [online Codex CLI suite](../../test/online/run-codex-cli.ts). The injection certification must additionally omit routing-field overrides so the test proves the injected `config.toml` and `token-model-catalog.json`, not merely the endpoint.
