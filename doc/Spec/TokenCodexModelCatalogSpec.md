...
- The current Responses conversion drops `text.verbosity`; therefore the catalog must not infer that capability merely from Codex defaults. `parallel_tool_calls` is consumed by the Responses Client Protocol and mapped onto Pi's neutral `parallelToolCalls` option; the selected Pi Provider/API adapter then applies or safely ignores/omits it, and reports a Provider-owned notice only when that adapter exposes such a channel. Codex does not read a model-catalog field for it, so the catalog carries no parallel-tool declaration.
