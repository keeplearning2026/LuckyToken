...
- The current Responses conversion drops `text.verbosity`; therefore the catalog must not infer that capability merely from Codex defaults. `parallel_tool_calls` is consumed by the Responses Client Protocol but the pinned upstream Pi 0.87 common options have no neutral representation for it, so Semantic Conversion omits the preference with a bounded Client-owned warning and leaves Pi/Provider defaults in force. Codex does not read a model-catalog field for it, so the catalog carries no parallel-tool declaration.
