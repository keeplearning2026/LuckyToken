# Token Semantic Conversion Architecture Specification
...
They may call the same pinned Pi AI dependency through an existing narrow infrastructure capability. They may also reuse a proven mechanism-only leaf utility that contains no Client Protocol semantics and owns no execution lifecycle. They do not share a Token Semantic Conversion kernel: each protocol module owns its own `onPayload`, projection termination, outcome collection, typed semantic failure, and Pi invocation orchestration.
...
Request correctness ends at the final Provider request, not at an intermediate Token object or Pi AI IR. Response correctness ends at the Client response and, for replay-required facts, at the next complete-history Provider request.
...
Its Interface, outcome types, errors, and lifecycle are private to the owning Client Protocol. Similar implementations may be locally duplicated. No protocol-neutral Token wrapper coordinates semantic execution for multiple Client Protocols.
