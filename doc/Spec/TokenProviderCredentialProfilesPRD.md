# Token Provider Credential Profiles PRD v1.5
...
2. Token's file credential store persists `Record<providerId, Credential>` in `auth.json`.
3. Token's `LiveCredentialAuthority` exposes one bounded `ProviderAuthStatus` row per Provider.
...
13. Pi `Models.refresh()` refreshes Provider model/catalog facts and may use the network. It does not change the active Token profile and is not required merely because `activeCredentialId` changed.
...
The current product forces users to overwrite working credentials, remember opaque key suffixes outside Token, and lose Provider access when the only credential expires or is rate-limited. It also weakens trust because a user who gives Token a secret cannot remove that secret through the same product surface.
...
Credential Profiles let Token become a reliable Provider access manager rather than a single-credential setup screen. The differentiating value is not merely storing more secrets; it is keeping authorized identities understandable, healthy, removable, and predictably selected.
...
- an operator running Token continuously for several clients or workflows;
...
8. understand when Token removed only local material versus revoked remote authorization.
...
Users must be able to see, name, disable, and remove every Token-managed credential before Token automatically selects among them.
...
After capture, Token never returns a complete API key, access token, or refresh token to Renderer, Control Plane projections, Activity, diagnostics, logs, or error messages.

## 5.4 Honest logout semantics

Token distinguishes local removal from Provider-side revocation. It never claims that an API key was revoked remotely merely because the local copy was deleted.
...
A **Provider Credential Profile** is one Token-managed credential belonging to one Provider. `authType` is Pi's two-branch authentication discriminant, not the product name or a claim about the credential payload's physical form.
...
- `identityHint` is optional, generated by Token, and never routing input. A literal key may use a safe masked suffix such as `•••• 7K2P`; a cloud profile, credential chain, service-account configuration, or opaque OAuth credential may have no safe hint.
- V1 does not claim a real OAuth account identity. The current Pi auth contract does not expose typed account metadata, and Token must not guess an email or account ID by parsing an opaque token.
...
2. Token shows the Provider-owned `api_key`-branch interaction without relabeling it as a literal key.
...
The destructive action is labeled `Disconnect from Token` and includes the Provider-declared OAuth method label.
...
V1 does not switch credentials for 401, 403, 5xx, network failures, OAuth refresh failures, storage failures, or any inferred quota condition. If the committed OAuth successor cannot be resolved because its silent token refresh fails, that retry stops with the auth failure: Token does not try a third profile, roll back the committed active pointer, start login, or open a browser. Failure in the selected Provider Native or Semantic Conversion lane never falls through to the other lane.

## 8.5 Cooling-down behavior

A valid explicit `Retry-After` (or an equivalent typed Provider fact) may mark the attempted profile `cooling_down` until the declared time. Without that evidence, Token excludes the profile only from the current request attempt set and does not invent a cooldown duration.
...
Consequently, Provider Native is neither semantic conversion nor blind header passthrough. It preserves the body semantics while rebuilding the Pi-owned transport envelope. Its only body-ordering exception is the closed Responses adjacency deferral, which relocates original developer message slices without rewriting any field, value, or content.

The first of two exceptions to literal body preservation is `(providerId=anthropic, api=anthropic-messages, authType=oauth)`. Current pinned Pi source changes the request body for this authentication mode by adding the required Claude Code system identity and projecting recognized Claude Code tool names consistently through tool definitions and related message references. Token mirrors only that confirmed OAuth differential inside the Anthropic Native lane. The second exception is the Provider Native Responses `operation === "responses"` tool-call adjacency deferral, which moves original `role=developer` message element slices inside `input`. Neither exception authorizes general Anthropic or Responses normalization, Pi AI IR conversion, invented defaults, malformed-body repair, or changes to unrelated fields.
...
5. leaves every other body field, value, and model-visible semantic unchanged, except that first-party Anthropic Messages under the captured OAuth Profile applies the exact pinned OAuth identity/tool-name projection defined above, and except the closed Responses adjacency deferral, which relocates original developer message element slices within the `input` array for `operation === "responses"`;
...
- Under the refresh lock, Token briefly acquires the Provider record lock to read the exact current payload and `credentialGeneration`, releases the record lock, runs the OAuth network refresh, then briefly reacquires the record lock to publish.
...
9. Credentials are for Provider access the user is authorized to use. Token does not describe selection as quota circumvention.
...
There is one authoritative representation. Token does not dual-write profile state into legacy `auth.json`, and Pi never reads the new physical shape directly.
...
- multiple Token-managed Profiles in Pi's `api_key` branch per Provider, including Provider methods that are not literal API keys;
- multiple Token-managed OAuth profiles per Provider when the Provider exposes OAuth login;
...
- Provider Native body preservation with boundary-required `model` projection and only the two closed body exceptions (the Anthropic OAuth differential and the Responses adjacency deferral);
...
- changing a Provider Native body field other than the boundary-required top-level `model` projection, the exact pinned Anthropic OAuth identity/tool-name differential, or the closed Responses tool-call adjacency deferral;
...
- expose `Remove from Token` for managed Provider credentials;
- expose `Disconnect from Token` for OAuth Profiles;
...
- preserve each Provider Native body except for model identity projection, the closed Responses adjacency deferral, and the closed Anthropic OAuth differential, then reconstruct its Pi Agent-compatible transport envelope;
...
- Provider Native changes any decoded request-body semantic other than the boundary-required top-level `model` projection, the closed Responses tool-call adjacency deferral, or the exact certified Anthropic OAuth identity/tool-name differential;
...
18. **Unsafe switching:** Upstream acceptance is uncertain or output has begun. Token returns the failure without selecting another profile or lane.
...
36. **Silent refresh failure:** A 429 switch commits OAuth Profile B, but B's bound token refresh fails. The request stops with a sanitized auth failure; Token does not try C, roll back B, invoke login, open a browser, or prompt the user.
...
40. **Default Provider Native body preservation:** For every claimed combination except first-party Anthropic OAuth and the closed Responses adjacency deferral, send a compatible Native request containing nested tools, instructions, metadata, extension fields, and a qualified model selector. The decoded upstream body differs only in the top-level `model` projection; for `operation === "responses"` it may additionally relocate original `role=developer` message element slices inside `input`; no other field, value, relationship, default, or extension is changed.
...
21. Token retains one Backend-lifetime Pi `Models` collection and presents only the exact operation binding through a composition-private Pi-compatible CredentialStore Adapter; Pi AI is not modified and does not know about sibling Profiles.
...
24. `Retry-After` or an equivalent typed fact may establish cooldown; Token invents no delay when it is absent.
...
38. First-party Anthropic `anthropic-messages` with a captured managed OAuth Profile is one of two Provider Native body-projection exceptions; the other is the closed Responses tool-call adjacency deferral for `operation === "responses"` described in the architecture spec. Its lane-owned implementation mirrors only the pinned Pi Agent OAuth-dependent Claude Code identity/tool-name differential, preserves unrelated semantics, and does not enter or reuse Pi AI IR/Pi Provider execution.
