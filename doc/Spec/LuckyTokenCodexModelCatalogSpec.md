...
Runtime contract: on `codex-cli 0.149.0`, the built-in `openai` provider may first attempt a WebSocket Responses connection even when the routed catalog says `prefer_websockets: false`. LuckyToken deliberately leaves WebSocket `/v1/responses` unsupported: it receives the existing `426 Upgrade Required` wire with code `websocket_transport_not_supported`, and Codex retries the turn over HTTP. The same listener always accepts the explicitly supported Codex Realtime `/v1/live` and `/v1/realtime` upgrade shapes. Other WebSocket paths retain the 426 behavior, non-WebSocket upgrades retain the prior close behavior, and ordinary HTTP requests remain unchanged.
