...
- [CommandCode model metadata](../../packages/provider-commandcode-private/src/models.ts) contains per-model context, modalities, reasoning status, and `thinkingLevelMap` data. For example, DeepSeek V4 Flash exposes `high/max`, Qwen 3.8 Max exposes `low/medium/xhigh`, Grok 4.5 exposes `low/medium/high`, and GPT-5.6 Luna exposes `low/medium/high/xhigh/max`.
