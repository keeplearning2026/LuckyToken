...
3. Token 的 Provider 配置 `models.json` 与 Pi 0.84.2 **schema 兼容**，但不是同一个文件，也不是同一种产品状态。Token 明确把自己的文件放在配置文件旁，绝不放进 Pi Agent 的 `~/.pi/agent/models.json`（`README.md:225-230`）；兼容基线固定为 Pi 0.84.2（`src/providers/pi-baseline.ts:3-19`）。复制 Token 的 Provider 配置到外部 Pi 会让 Pi 直接配置上游 Provider，绕过 Token，且可能跨越凭据边界。
...
- “已同步”：目标文件已安全写入并验证；
- “当前 Pi 会话已刷新”：Token 没有 Pi 的运行时管理通道，不能从文件写入本身保证。

面向用户可说明“无需重启；下次打开 `/model` 或触发模型刷新时生效”。

## `favorite` 的契约边界

Pi 的模型 schema 没有 `favorite`（`pi-agent/packages/coding-agent/src/core/model-config.ts:157-171`）。0.84.2 没有设置严格的 `additionalProperties: false`，Token 的固定基线测试也记录了未知字段会通过校验、组合时忽略的宽松行为（`test/unit/models-json-schema.test.ts:153-178`）。加载器还会深拷贝整个 Provider 对象，因此未知嵌套字段暂时留在内存配置快照（`pi-agent/packages/coding-agent/src/core/model-config.ts:279-284`）；但有效模型构造器只返回已知字段（`pi-agent/packages/coding-agent/src/core/provider-composer.ts:150-165`）。
...
结论：不要把 Token 的领域字段塞进一个外部产品的宽松 JSON schema。

## Token 自有状态是否适合保存 `favorite`

适合，但当前代码不能只在文件里手工加一个布尔值。

Token 有两份完全不同的文件：

- `models.json`：Token 自己的 Provider/模型运行配置；schema 对齐 Pi。
- `public-models.json`：Public Model 的 endpoint、Provider 开关、模型别名/target/开关。README 明确区分二者（`README.md:65-77`），应用把 `public-models.json` 放在 Token `models.json` 同目录（`src/application.ts:575-592`）。
...
不要复制 Token 自己的 `models.json`。应从“收藏的 Public Model + 有效运行时模型事实 + Token 接入端点”生成一个专用 Provider，例如：
...
```json
{
  "providers": {
    "Token": {
      ...
      "apiKey": "<Token 的 OpenAI Responses 客户端令牌>",
      "models": [
        {
          "id": "provider/alias",
          "name": "Display name",
...
1. `models[].id` 使用 Token 对外接受的 alias，因为 Pi 文档明确 `id` 会原样传给 API（`pi-agent/packages/coding-agent/docs/models.md:197-202`）。
2. 只投影全局收藏且当前可注入的模型；不存在 `full/all` 模式。
3. `reasoning`、`thinkingLevelMap`、`input`、`contextWindow`、`maxTokens` 等模型能力来自 Token 的有效目标模型；否则 Pi 的默认值可能错误。
4. `baseUrl`、`api`、认证和 `compat` 描述的是 **Pi Agent → Token** 这一条客户端协议边界，不能复制目标上游 Provider 的 endpoint、credential 或 compat。
5. 不写 `favorite`，也不写任何仅属于 Token UI/控制面的字段。
...
```text
Token public-models.json
  └─ favorite（全局用户偏好，最多 10 个；Token Authority 所有）
       └─ Pi Integration 读取收藏 + 有效模型事实
            └─ 生成仅含 Pi 已知字段的 providers.Token
                 └─ CAS/锁/原子合并到 ~/.pi/agent/models.json
```

这条信息流保持三个边界清楚：收藏不改变 Token 发布/请求语义；外部 Pi 不认识 Token 私有字段；同步只修改用户明确授权的一个集成子树。
