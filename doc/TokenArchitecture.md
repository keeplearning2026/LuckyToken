...
本文回答六个问题：Token 最终提供什么功能；一次请求如何完成；系统由哪些
...
| Token（本地认证） | Agent 进入 Token 的门票，不是模型输出消耗的 token 数量 |
...
可以把 Token 想成一家“翻译兼转运服务”。Agent 按 Anthropic 或 OpenAI
Responses 格式下单，而当前外部模型服务使用 CommandCode 的格式。Token 不替模型思考，
只负责验票、翻译、转交、确认结果完整，再按 Agent 能理解的格式送回来。

```mermaid
flowchart LR
    A["Agent<br/>说 Anthropic 格式"] --> B["Token<br/>验票与翻译"]
    B --> C["Pi 标准中间语义"]
    C --> D["CommandCode 适配员"]
    D --> E["真实模型服务"]
    E --> D --> C --> B --> A
...
Token 当前是一个本地模型协议桥接服务。它在 loopback TCP 上提供 Anthropic
Messages 与 OpenAI Responses endpoints，让 Claude Code、Codex CLI 等 Agent 使用
本地 Data Plane 固定为 loopback，不需要 Token 自己的 per-protocol client token。
...
> Token 像中间服务台：Agent 不需要知道远端供应商的专用格式，远端供应商也不
> 需要知道客户端使用的是哪种 Agent。
...
- Data Plane 固定为 loopback-only，不维护 Token global/project client-token；
...
- 支持 `output_config.effort` → Pi reasoning 映射；
...
- 在 Token 中重新实现一套与 Pi 平行的通用 LLM IR。
...
Token 不是一个什么都做的巨大模块，而像一条先分流、再由各 lane 自己完成任务的流水线。HTTP/Runtime 先确定 route/profile 与 eligible lane；只有 Semantic Conversion 才进入 Client Protocol → Pi → Provider 翻译链。
...
> **小白理解：** 现在 Token 有两个长期生命周期：Backend 负责“服务本身活多久”，Electron Main 负责“桌面如何找到、连接并管理这个 Backend”。两边通过 Control Plane discovery/IPC 相遇，但 Desktop 不拥有 Backend 的业务状态。
...
Node 22 WHATWG `Request/Response`，不建立 Token 自定义 HTTP DTO。
...
Token 不再给本地 Agent 发一套 global/project client token。当前安全相关 credential 分成三个独立生命周期：
...
2. Provider credential
   ~/.Token/pi/credential-profiles/<providerId>.json
...
Model Data Plane 固定绑定 loopback 地址。Anthropic/OpenAI Responses 的普通 semantic-conversion request 不需要 Token 自己的 client-token 文件，也没有 `client-token` CLI。

这并不表示所有 incoming credential 都被忽略：native preservation lane 可以把 client wire 上的 credential 当作该 lane 自己的 transport/auth fact，但它不能被提升成全局 Token credential。

## 4.2 Codex Direct Mode caller envelope

Codex Direct Mode 不读取 `auth.json`、不解析 JWT，也不建立 Token credential authority。调用方的 Authorization、account ID、Cookie、API-key 类端到端头和原始 query 保持在该 Lane 内并转发到固定 Codex upstream，由 upstream 决定认证结果。Direct Mode 只重建 Host、长度、连接级头和 WebSocket 握手头；这些 credential 不进入 Pi Models、Provider credential store、Provider Native 或 Semantic Conversion。
...
> 也可能携带实验或 SDK headers。Token 这里只核对版本；遇到陌生版本会拒绝，
...
该 Client Protocol 的短生命周期输出不是新的通用 IR；实际形状以
`src/protocols/anthropic/semantic/invocation.ts` 为准。`AnthropicSemanticInvocation`
只包含 `pi.context` + `pi.options` 与 `reasoning`；`AnthropicConversionResult`
另外提供 `selector`、`client.renderState` 和 `client.notices`：
...
interface AnthropicConversionResult {
  ...
  client: {
    renderState: AnthropicRequestRenderState;
...
supplement，也不进入 Provider payload projector。未声明的 `context_management`
及未来未知字段同样生成 bounded `unclaimedTopLevelKeys` 警告（上限 8 条），不校验、
不进入 Pi 状态。content block 与 tool definition 的额外控制字段（`citations`、
`caller`、`type`、`allowed_callers`、`defer_loading` 等）按"只读所需"原则忽略。
...
## 5.8 Pi execution commit — `src/execution.ts`（经 `semantic/execution.ts`）
...
neutral execution capability `src/execution.ts` → `Models.streamSimple()`。顶层
...
flowchart TD
    ...
    SE --> E["src/execution.ts → Models.streamSimple()"]
    H --> RS["response.ts"]
    RS --> W["wire.ts JSON"]
...
Token 两边会不断变化：左边将来可能有 Anthropic、OpenAI Responses 等不同
...
stream behavior。Token 不在两者之间加 Manager/Registry。
...
- `id`：官方权威 id（如 `deepseek/deepseek-v4.1-flash`、`Qwen/Qwen3.8-Max`）；
...
默认模型仍是 `deepseek/deepseek-v4.1-flash`（`COMMANDCODE_DEFAULT_MODEL_ID`）。

### 6.2.1 用户自定义 Provider — `models.json`（新增）

用户可通过 `pi.modelsJson` 配置（默认与 config 同目录的 `models.json`，桌面布局
即 `~/.Token/models.json`；始终是 Token 自有数据目录，绝不隐式读写
...
`src/providers/models-json.ts` 是 Token 自有解析器（最小 schema 子集，不
...
| 配套文件 | `.Token/pi/credential-profiles/<providerId>.json` |
...
> **小白理解：** `.Token/config.json` 是部署地址簿：Data Plane 端口、各 Client
> Protocol 的 conversion/state policy、Provider package 配置、Pi 文件夹和请求限额。
> 它不保存 Token client token，也不承担 Backend singleton/discovery；后两者固定由
> current-user application state root 派生。

```ts
loadTokenCliConfig(path): Promise<TokenCliConfig>
```

严格文件 `.Token/config.json` 的当前结构：
...
| 输入 | 输出/行为 |
| --- | --- |
| `piDirectory` / `modelsJsonPath` | 定位 per-Provider Profile records 与 Token-owned models catalog |
...
> **小白理解：** CLI 只是进程入口，不再自己组装整套 Backend。`serve` 负责请求“启动或附着当前用户的 Token Backend”；`control ...` 连接已经运行的 Control Plane，并通过 Backend authority 管理 Provider 凭证。

Top-level CLI 是 adapter/shell，不是 Backend composition root：

| 命令 | 使用模块 | 产生的外部效果 |
| --- | --- | --- |
| `serve --config ...` | `startTokenApplication()` | 取得/附着 current-user Backend authority；运行或附着 Backend |
...
`serve` 不接受自定义 descriptor。生产 singleton 固定使用 `~/.Token/instance.sqlite`，matching discovery 固定发布到 `~/.Token/control-plane.json`；只有 `control ...` 客户端命令把 `--descriptor` 当作连接导航参数。integration tests 若需要隔离实例，应通过独立 HOME 或内部 composition dependency 隔离，而不是改变生产 instance domain。
...
Serve 获得 `RunningTokenApplication` 后只等待 process signal 或 application-owned exit。SIGINT/SIGTERM 调用 Backend Application 的 graceful shutdown seam；具体 Data Plane drain、Control Plane/publication、stores 与 `InstanceLease` 的关闭顺序由 `application.ts` 拥有，CLI 不重复实现。

## 6.8 Pi 文件与 ownership 关系

> **小白理解：** Pi 目录保存 Provider credential 与动态 catalog cache；Token 的
> `models.json`/`public-models.json` 仍由 Token application state root 单独拥有。

```mermaid
flowchart TB
    D[".Token/pi/"] --> A["credential-profiles/<provider>.json\nProvider Profiles"]
...
Pi `AssistantMessageEventStream`。Client Protocol 只输出 Pi `Context`/options；
通用 `src/execution.ts` 经 `Models.streamSimple()` 执行，并由 Pi Models 选择已注册
Provider。该包不拥有 Client Protocol conversion，也完全不知道请求最初来自 Anthropic
还是 OpenAI Responses。
...
> **小白理解：** 当前 CommandCode Private 不再读取 Token/Pi 的项目目录，也不扫描
> filesystem/Git。上游 wire 仍要求一个 `config` object，所以 Provider 只构造协议要求的
> fixed empty representation；它是兼容性字段，不是 Token 的 workspace model。
...
> 和网络地址，再生成完整订单并重新验货，最后把它冻结。Pi public `onPayload` hook
> 若由外部调用方提供，可在这个 Provider adapter 边界观察或替换候选 payload，但不能
> 偷换已经确认的模型/session。一次逻辑任务只封箱一次，重试仍发送同一权威订单。
...
→ optional Pi public `onPayload` hook（Provider adapter 观察；非 Client Protocol /
  Semantic Conversion 创建的 seam）
→ JSON serialize
→ parse and validate against captured authority
→ freeze endpoint/headers/bodyText/signal/fetch
```

`validateCommandCodeRequest()` 在 Pi public hook 与 serialization 之后重新检查
model ID、session、fixed config、permission、image/reasoning capabilities、
message/tool lifecycle 和 closed-world fields。因此 `onPayload` 若由 Provider
测试或低层基础设施观察方通过 Pi options 提供，可以在 Pi Provider adapter 内参与
payload transform，但 Client Protocol 与 Semantic Conversion 不创建、不依赖它，它也
不能偷偷改写 Provider authority。
...
```mermaid
flowchart TD
    Models["Pi Models"] --> Provider["registered Pi Provider<br/>streamSimple boundary"]
...
`@token/provider-commandcode-goat` 是另一条独立的 Semantic Conversion Provider：
...
哪些正式入口使用 Token。Singleton lock、Control Plane discovery、Provider
...
module 不应打开/复制/删除它。生产 backup 使用显式 allowlist，不扫描整个 `.Token/`。
...
Subpath `Token/protocols/anthropic`：
...
| `src/application.ts` | `startTokenApplication()` → running/attached Backend | CLI / packaged Backend entry | InstanceAuthority、management authorities、Control Plane、Data Plane supervisor | Backend Application integration + ownership E2E |
...
| `src/server.ts` | `startTokenHttpServer()` → running server | Data Plane composition/tests | Node HTTP/stream、`TokenRuntime` | local-http-server / atomic delivery |
| `src/runtime.ts` | `createTokenRuntime()` → `handle(Request)` | composition/programmatic | `http.ts` | Client Protocol boundary tests |
...
| `packages/provider-commandcode-private/src/provider.ts` | internal factory、Pi→wire conversion、request preparation（package root 只导出 `providerPackage`） | Provider Package entry | fixed config helper、attempts、semantic、JSON、Pi helpers | golden request、payload authority、boundary/tools/history tests |
...
`CommandcodeAPIKey.txt`，默认模型是 `commandcode-private/deepseek/deepseek-v4.1-flash`，保存脱敏的完整
wire evidence。
...
Capturing wrapper 位于 Provider 的 external transport boundary，不 mock Token 内部
...
`@token/provider-contract/package`。若它属于产品内置能力，应加入 bundled Provider
...
1. 更新 npm dependency/审查同版本 `pi-agent/packages/ai` upstream snapshot；
2. 不在 vendored package 中加入 Token patch；
