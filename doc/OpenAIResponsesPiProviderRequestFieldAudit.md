...
Current boundary banner: the projector and Supplement architecture described below was the historical baseline. The current clean upstream Pi 0.87.0 boundary keeps that architecture deleted. The Responses converter maps consumed semantics only into Pi `Context` and upstream public Pi common options; non-structural controls without a public Pi representation are omitted with bounded warnings, while structural semantic loss still fails. The baseline prescriptions below are evidence, not current implementation instructions, and do not authorize a protocol-owned projector, registry, Supplement, source-protocol × target-API matrix, payload seam, or `onPayload`.

Current correction: Semantic Conversion now uses demand-driven extraction. Historical target-capability evidence below remains useful, but a source field with no declared Responses consumer is not read or validated merely because it appears in the official request schema. Clean upstream Pi 0.87 exposes `toolChoice` only as `auto` / `none` and has no common `parallelToolCalls`; required/named/parallel controls are therefore omitted with bounded warnings.
...
For a compatible Responses client and Responses Provider, Token's Native Preservation lane should forward the authoritative raw body instead of relying on these partial semantic mappings.
